...
def clean_opening_string(opening_str):
    ...
    return {"gross": int(currency_str), "screens": int(screens_str)}
...
def crawl_movies_list_data():
    ...
    driver.get(url)
    movies_list_len = driver.find_element(
        By.XPATH, '//*[@id="table"]/div/table[2]/tbody/tr[last()]/td[1]'
    ).text
    movies_list_len = int(movies_list_len)
    for i in range(2, movies_list_len + 2):
        movie_url_href = driver.find_element(
            By.XPATH, f'//*[@id="table"]/div/table[2]/tbody/tr[{i}]/td[2]/a'
        ).get_attribute("href")
...
def crawl_movie_data(url):
    ...
    DOMESTIC_GROSS_XPATH = (
        '//*[@id="a-page"]/main/div/div[3]/div[1]/div/div[1]/span[2]/span'
    )
    ...
    for i in range(2, properties_count - 1):
        property_name = driver.find_element(
            By.XPATH, f'//*[@id="a-page"]/main/div/div[3]/div[4]/div[{i}]/span[1]'
        ).text
        property_value = driver.find_element(
            By.XPATH, f'//*[@id="a-page"]/main/div/div[3]/div[4]/div[{i}]/span[2]'
        ).text
...
def main_mojo(filename):
    ...
    print(
        f"{GREEN_BG_BLACK_TEXT_BOLD}CRAWL MOVIES LIST DATA{RESET} Total time cost: {YELLOW_TEXT}{movies_url_list_time_cost:.2f}s{RESET_TEXT}"
    )
    ...
    for movie_url in movies_url_list:
        ...
        print(
            f"MOVIE DATA (Cost: {YELLOW_TEXT}{movie_data_time_cost:.2f}s{RESET_TEXT}) Title: {movie_data['movie_name']}"
        )
    ...
    print(
        f"{GREEN_BG_BLACK_TEXT_BOLD}CRAWL MOVIE DATA{RESET} Total time cost: {YELLOW_TEXT}{movies_list_data_time_cost:.2f}s{RESET_TEXT}, average time cost: {YELLOW_TEXT}{movies_list_data_time_average:.2f}s{RESET_TEXT}"
    )
